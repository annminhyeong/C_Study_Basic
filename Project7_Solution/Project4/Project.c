...
{	
	...
	{
		tax = BASE1 * RATE1 + (BASE2 - BASE1) * RATE2 + (BASE3 - BASE2) * RATE3 
			+ (BASE4 - BASE3) * BASE4 + (income - BASE4) * RATE5;
	}
	else if(income <= BASE6)
	{
		tax = BASE1* RATE1 + (BASE2 - BASE1) * RATE2 + (BASE3 - BASE2) * RATE3
			+ (BASE4 - BASE3) * BASE4 + (BASE5 - BASE4) * BASE5 + (income - BASE5) * RATE6;
	}
	else //else if(income > BASE6)
	{
		tax = BASE1 * RATE1 + (BASE2 - BASE1) * RATE2 + (BASE3 - BASE2) * RATE3
			+ (BASE4 - BASE3) * BASE4 + (BASE5 - BASE4) * BASE5 + (BASE6 - BASE5) * BASE6
			+ (income - BASE6) * RATE7;
	}
	printf("Tax is %f\n", tax);
	printf("Your income after deduction %f\n", income - tax);
