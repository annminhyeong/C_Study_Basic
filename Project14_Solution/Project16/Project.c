...
void write_books2(const char* filename, const struct book* books, int n);
struct book* read_books(const char* filename, int* n);
void read_books2(const char* filename, struct book** books_dptr, int* n);
void read_books3(const char* filename, struct book** books_dptr, int* n_ptr);
...
{	
	...
	write_books2("books.dat", my_books, n);
	free(my_books);
	n = 0;
	printf("Done.\n");
	...
	read_books3("books.dat", &my_books, &n);
...
void write_books(const char* filename, const struct book* books, int n)
{
	...
	if (file == NULL)
	{
		fputs("Can't not open file.", stderr);
		exit(1);
	}
